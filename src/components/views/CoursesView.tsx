...
export const CoursesView: React.FC = () => {
  ...
  const [adLinkCourse, setAdLinkCourse] = useState<Course | null>(null);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
                      ...
                      <div className="relative">
                        ...
                        {isMenuOpen && (
                          <div className="absolute right-0 top-8 w-44 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-20 text-xs animate-in zoom-in-95 duration-100">
                            ...
                            <button
                              onClick={() => {
                                setAdLinkCourse(course);
                                setActiveMenuCourseId(null);
                              }}
                              className="w-full px-3 py-1.5 text-left text-indigo-700 hover:bg-indigo-50 flex items-center space-x-2 font-bold"
                            >
                              <Target className="w-3.5 h-3.5 text-indigo-600" />
                              <span>🎯 Ad Campaign Link</span>
                            </button>
      ...
      {/* Ad Campaign Landing Link Generator Modal */}
      {adLinkCourse && (
        <AdLinkGeneratorModal
          isOpen={!!adLinkCourse}
          onClose={() => setAdLinkCourse(null)}
          course={adLinkCourse}
          onOpenCustomizer={() => {
            const courseToCustomize = adLinkCourse;
            setAdLinkCourse(null);
            setCustomizerCourse(courseToCustomize);
          }}
        />
      )}

      {/* Course Landing Page Content & Variant Editor Modal */}
      {customizerCourse && (
