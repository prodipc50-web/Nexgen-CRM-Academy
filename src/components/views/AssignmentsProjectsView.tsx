...
export const AssignmentsProjectsView: React.FC = () => {
  ...
  const [editStatus, setEditStatus] = useState<'Submitted' | 'Graded'>('Submitted');
  ...
  const handleOpenEditSubmission = (sub: AssignmentSubmission) => {
    setEditingSubmission(sub);
    setEditProjectUrl(sub.projectUrl || '');
    setEditGithubUrl(sub.githubUrl || '');
    setEditNotes(sub.notes || '');
    setEditStatus((sub.status as 'Submitted' | 'Graded') || 'Submitted');
    setEditMarks(sub.marksObtained || 0);
    setEditFeedback(sub.feedback || sub.trainerFeedback || '');
  };

  const handleSaveEditSubmission = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingSubmission) return;
    updateAssignmentSubmission(editingSubmission.id, {
      projectUrl: editProjectUrl,
      githubUrl: editGithubUrl,
      notes: editNotes,
      status: editStatus,
      marksObtained: editStatus === 'Graded' ? editMarks : undefined,
      feedback: editFeedback,
      trainerFeedback: editFeedback
    });
  ...
  const handleConfirmDeleteSubmission = () => {
    if (submissionToDelete) {
      deleteAssignmentSubmission(submissionToDelete.id);
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end space-x-1.5">
                          <button
                            ...
                            className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg font-bold text-xs transition-colors"
                            title="Grade / Review"
                          >
                            Grade
                          </button>
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-150">
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-bold">Edit Project Submission</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveEditSubmission} className="p-6 space-y-4 text-xs">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-[11px] text-slate-500 font-medium">Student</span>
                  <div className="font-bold text-slate-900">{editingSubmission.studentName}</div>
                </div>
                <div className="text-right">
                  <span className="text-[11px] text-slate-500 font-medium">Student Code</span>
                  <div className="font-mono font-bold text-indigo-600">{editingSubmission.studentCode}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-100">
                <div>
                  ...
                  <select
                    value={editStatus}
                    onChange={e => setEditStatus(e.target.value as 'Submitted' | 'Graded')}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium"
                  >
                ...
                <div>
                  <label className="block text-slate-700 font-bold mb-1">Marks Obtained</label>
                  <input
                    type="number"
                    min={0}
                    max={100}
                    value={editMarks}
                    onChange={e => setEditMarks(Number(e.target.value) || 0)}
                    disabled={editStatus !== 'Graded'}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold disabled:opacity-50"
                  />
                </div>
              </div>
              ...
              {editStatus === 'Graded' && (
                <div>
                  <label className="block text-slate-700 font-bold mb-1">Trainer Feedback</label>
                  <textarea
                    rows={2}
                    placeholder="Feedback, areas of improvement..."
                    value={editFeedback}
      ...
      {submissionToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Are you sure you want to remove project submission by <strong>{submissionToDelete.studentName}</strong>?
            </p>
            <div className="mt-5 flex items-center justify-center space-x-3">
              <button
                type="button"
