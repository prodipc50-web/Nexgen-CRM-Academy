...
export const SettingsView: React.FC<SettingsViewProps> = ({ onViewPublicWebsite }) => {
  const {
    currentUser,
    auditLogs,
    resetToSeedData,
    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex border-b border-slate-200 text-xs font-bold space-x-6 overflow-x-auto">
        ...
        >
          ...
          <span>Fraud Protection & OTP</span>
        </button>
        ...
        >
          ...
          <span>Meta Pixel & Ads Tracking</span>
        </button>
      ...
      {/* TAB: MARKETING & META PIXEL ENGINE */}
      {activeTab === 'marketing' && (
        <MarketingAnalyticsDashboard onViewPublicWebsite={onViewPublicWebsite} />
      )}

      {/* TAB: THEME & BRAND ENGINE */}
      {activeTab === 'theme' && (
        <ThemeEditor onViewPublicWebsite={onViewPublicWebsite} />
      )}
      ...
      {activeTab === 'profile' && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-2xs max-w-3xl space-y-6 text-xs animate-in fade-in duration-150">
          ...
          {/* Logo Uploader & Crop Studio */}
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              <label className="font-bold text-slate-800 block text-sm">
                Official Institute Logo & Branding (লোগো ক্রপ, রিসাইজ ও কাস্টমাইজেশন)
              </label>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
              <div className="flex items-center space-x-3.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Live on header, website, ID cards, receipts, and certificates</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {localStorage.getItem('NEXGEN_OFFICE_ACADEMY_CUSTOM_LOGO') && (
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm('Restore the official default Nexgen shield emblem?')) {
